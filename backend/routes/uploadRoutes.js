...
router.post("/upload", authMiddleware, upload.single("file"), async (req, res) => {
  try {
    ...
    // Send extracted text to Python via stdin
    ...
    pythonProcess.on("close", (code) => {
      ...
      try {
        ...
        // Save to MongoDB with skillsSection and experienceSection only
        const newResume = new Resume({
          userId: req.user.id,
          fileUrl,
          extractedText,
          skillsSection: nlpData.skillsSection || "",
          experienceSection: nlpData.experienceSection || "",
        });

        newResume.save()
          .then(() => res.status(200).json({ message: "File uploaded and processed successfully", fileUrl, nlpData }))
